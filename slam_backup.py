...
import numpy as np
from utilities.utils_draw import draw_torch_image, visualize_matches, create_histogram_image
...
if __name__ == "__main__":
    ...
    kShowDebugImages = config.ShowDebugImages

    # USING SUPERPOINT AND LIGHTGLUE FOR FEATURE EXTRACTION AND MATCHING
   ...
   # Loading Groundtruth trajectory
    ...
    while True: 
        ...
        if img_id > starting_img_id + kNumFramesAway - 1:
            ...
            if kNumFramesAway != 1: 
                ref_prev_same = False
                ref_img = dataset.getImage(img_id - kNumFramesAway)
                ref_depth_img = dataset.getDepth(img_id - kNumFramesAway) * depth_scale
