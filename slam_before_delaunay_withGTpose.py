...
if __name__ == "__main__":
    ...
    ## VISUALIZE THE GROUND TRUTH TRAJECTORY WITH RGBD POINT CLOUDS
    ...
    # Create trajectory line set and accumulate point clouds
    points = []
    lines = []
    for frame_idx, pose in enumerate(gt_poses):
        if frame_idx % 10 == 0:  # Only show every 10th pose
            timestamp = gt_timestamps[frame_idx]
            
            # Get RGB and depth images using frame index directly
            rgb = dataset.getImage(frame_idx)
            depth = dataset.getDepth(frame_idx) * depth_scale
            ...
            if rgb is not None and depth is not None:
                ...
                point_cloud = depth2pointcloud(
                    ...
                    max_depth=10.0,  # Limit depth for better visualization
                    min_depth=0.1
                )
                
                # Convert to Open3D point cloud and add to visualizer
            ...
            # Add coordinate frame for this pose
            frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.2)
            frame.transform(pose)
            vis.add_geometry(frame)
    
    # Create and add trajectory line set
    if points:
        trajectory = o3d.geometry.LineSet()
        trajectory.points = o3d.utility.Vector3dVector(np.array(points))
        trajectory.lines = o3d.utility.Vector2iVector(np.array(lines))
    ...
    opt.point_size = 1.0  # Smaller point size for better density
